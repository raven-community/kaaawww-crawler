...
function start() {
  ...
  const db = level(cwd + '/databases/' + network_name, {
    valueEncoding: 'json',
  ...
  app.get('/', function(req, res) {
    ...
    recentConnections(1000 * 60 * 60 * hours)
      .on('data', function(connection) {
        ...
        if (
          host2lastconnection[key] === undefined ||
          connection.connectTime > host2lastconnection[key].connectTime
        ) {
      ...
      .on('close', function() {
        let css =
          '<head><script>' +
          gotoURL +
          '</script><style>body {  background: #000; } ' +
          menuCSS +
        ...
        let home =
          ...
          '</div>';
        res.send(css + '<body>' + menu + home + '</body>');
      });
  });
  ...
  app.get('/charts/bar-v', function(req, res) {
    ...
    recentConnections(1000 * 60 * 60 * hours)
      ...
      .on('close', function() {
        let hostList = Object.keys(host2lastconnection).filter(
          host => host2lastconnection[host].success
        );
        let hostData = Object.values(host2lastconnection).filter(obj => {
        ...
        subversion = subversion.sort((a, b) =>
        ...
        Object.keys(subversionList)
          ...
          .forEach(
            sortedData =>
              (subversionListSorted[sortedData.key] = JSON.parse(
        ...
        Object.keys(countryList)
          ...
          .forEach(
            sortedData =>
              (countryListSorted[sortedData.key] = JSON.parse(
                JSON.stringify(sortedData)
                  .replace('{"key":"' + sortedData.key + '"', '')
                  .replace(',"value":', '')
                  .slice(0, -1)
        ...
        Object.keys(ispList)
          ...
          .forEach(
            sortedData =>
              (ispListSorted[sortedData.key] = JSON.parse(
                JSON.stringify(sortedData)
                  .replace('{"key":"' + sortedData.key + '"', '')
                  .replace(',"value":', '')
                  .slice(0, -1)
        ...
        for (i in subversionCount) {
          allinfo.count.subversion =
            allinfo.count.subversion + subversionCount[i];
        }
        ...
        for (i in subversionListSorted) {
          let fixed =
            (subversionListSorted[i].count / allinfo.count.subversion) * 100;
          fixed = fixed.toFixed(2);
          subversionListSorted[i].percent = fixed;
        }
        for (i in countryListSorted) {
          let fixed =
            (countryListSorted[i].count / allinfo.count.country) * 100;
          fixed = fixed.toFixed(2);
  ...
  app.get('/charts/pie', function(req, res) {
    ...
    recentConnections(1000 * 60 * 60 * hours)
      ...
      .on('close', function() {
        ...
        let ispListValues = [];
        for (i in subversionListSorted) {
          subversionListKeys.push(
            '"' + i + '(' + subversionListSorted[i]['count'] + ')"'
          );
          subversionListValues.push(subversionListSorted[i]['count']);
        }
        for (i in countryListSorted) {
          countryListKeys.push(
            '"' + i + '(' + countryListSorted[i]['count'] + ')"'
          );
          countryListValues.push(countryListSorted[i]['count']);
        }
        for (i in ispListSorted) {
    ...
    recentConnections(1000 * 60 * 60 * hours)
      ...
      .on('close', function() {
        ...
        let html =
          ...
          '},' +
          'cells: {' +
          `values: [[${countryListKeys.slice(
            0,
            10
          )}],[${countryListValues.slice(0, 10)}],[${countryListPercents.slice(
            0,
            10
          )}]],` +
          ...
          '},' +
          'cells: {' +
          `values: [[${ispListKeys.slice(0, 10)}],[${ispListValues.slice(
            0,
            10
          )}],[${ispListPercents.slice(0, 10)}]],` +
          "align: 'center'," +
          "line: {color: 'black', width: 1}," +
          "fill: {color: '#696969'}," +
          "font: {family: 'Arial', size: 13, color: ['#333']}" +
  ...
  app.get('/node_count', function(req, res) {
    ...
    recentConnections(1000 * 60 * 60 * hours)
      ...
      .on('close', function() {
        ...
        let hostSuccess = Object.keys(host2lastconnection).filter(
          host => host2lastconnection[host].success
        ).length;
  ...
  app.get('/node_list', function(req, res) {
    ...
    recentConnections(1000 * 60 * 60 * hours)
      ...
      .on('close', function() {
        ...
        hostList = hostList.sort(function(a, b) {
          return b.version - a.version;
        });
        hostList = tableify(hostList).replace(
          'host',
          'host  (' + hostCount + ')'
        );
        res.send(css + menu + hostList);
      });
  });
  ...
  app.get('/connections/:host_ip.csv', function(req, res) {
    ...
    connectionsByHost(ip)
      .on('data', function(connection) {
        ...
        let columns = [readableDate, connection.host, connection.port];
    ...
    recentConnections(1000 * 60 * 60 * hours)
      ...
      .on('close', function() {
        ...
        for (i = 0; i < hostList.length; i++) {
          ...
          if (hostList[i].location) {
            escapeLocation = hostList[i].location.replace(/'/g, "\\'");
          }
          peerInfo.push(
            "{host: '" +
              hostList[i].host +
          ...
          if (hostList[i].country) {
            Highlight.push(hostList[i].country);
          }
        }
        let count = {};
  ...
  function add_connection2data(connection) {
    ...
    if (data.hour2first_and_last_connection_time[hours_ago] === undefined) {
      data.hour2first_and_last_connection_time[hours_ago] = {
        min: connectTime,
        max: connectTime
      };
    } else {
      if (
        connectTime < data.hour2first_and_last_connection_time[hours_ago].min
      ) {
        data.hour2first_and_last_connection_time[hours_ago].min = connectTime;
      }
      if (
        connectTime > data.hour2first_and_last_connection_time[hours_ago].max
      ) {
    ...
    if (
      connection.success &&
      (data.hostdata.host2lastconnection[host] === undefined ||
        data.hostdata.host2lastconnection[host].connectedTime <
      ...
      db.createReadStream({
        ...
        .on('data', function(data) {
          ...
          ops.push({
            type: 'put',
            key:
              connection_by_time_prefix +
              integer2LexString(connection.connectTime) +
              '/' +
              connectionId,
            value: connection
          });
  ...
  function shift_data_one_hour() {
    ...
    Object.keys(data.hostdata.host2lastconnection).forEach(host => {
      ...
      let last_connect_hour = Math.floor(
        lastConnection.connectTime / (1000 * 60 * 60)
      );
      ...
      for (let i = 24 * 30; i > 0; i--) {
        data.hostdata.host2active[host].set(
          i,
          data.hostdata.host2active[host].get(i - 1)
        );
      }
  ...
  function host2lastAddr(duration) {
    ...
    db.createReadStream({
      ...
      .on('data', function(data) {
        if (duration !== undefined && currentTime - data.value > duration)
          return;
        event2callback['data'](data);
      })
      .on('error', function(err) {
  ...
  function createQueue(callback) {
    ...
    Promise.all([host2LastConnectionPromise, host2timePromise]).then(function(
      values
    ) {
  ...
  function saveConnection(connection, connectionId) {
    ...
    if (connection.version != 70015) {
      add_connection2data(connection);
      const ops = [
        {
          type: 'put',
          key: connection_prefix + connectionId,
          value: connection
        },
        {
          type: 'put',
  ...
  function connectToPeers() {
    ...
    let currentTime = new Date().getTime();
    while (
      failed_connections_queue.length > 0 &&
      currentTime - failed_connections_queue[0] > 1000 * 60
    ) {
    ...
    let status =
      'queue: ' +
      queue.length +
      ', failed_connections_queue: ' +
      failed_connections_queue.length +
      ', concurrent_connections: ' +
      concurrent_connections;
    if (queue.length > 0 && queue[0].nextConnection > currentTime) {
      status +=
        ', next action in ' +
        Math.floor((queue[0].nextConnection - currentTime) / 1000) +
        ' seconds.';
    }
    ...
    if (queue.length > 0) {
      let nextActionTime =
        currentTime - lastConnectTime > 1000 * 60 * 1
          ? 0
          : queue[0].nextConnection;
      if (
      ...
      db.batch(ops, function (err) {
        ...
        peer.on('ready', function() {
          ...
          else ISP = ISP.autonomous_system_organization;
          if (geo) {
          ...
          if (country) {
            ...
            if (regionName) {
              regionName = regionName.regions.filter(obj => {
                return obj.shortCode === geo.region;
              });
              if (regionName[0]) {
          ...
          addrTimeout = setTimeout(function() {
            console.log(
              'No addr message withing ' +
                stay_connected_time / 1000 +
                ' seconds'
            );
            peer.disconnect();
          }, stay_connected_time);
        ...
        peer.on('addr', function(message) {
          ...
          message.addresses.forEach(function(address) {
            ...
            let obj = {
              ...
              addrMessageId: addrMessageId,
              timestamp: addrTimeStamp,
              ip: address.ip,
              port: address.port,
              time: address.time.getTime()
            };
            ...
            db.get(key, function(err, value) {
              const ops = [];
              if (
                addr_db_ttl === undefined ||
                addr_db_ttl === 0 ||
                Math.max(0, addrTimeStamp - address.time.getTime()) <
                  addr_db_ttl
              ) {
                ops.push({
                  type: 'put',
                  key: addr_prefix + addressId,
                  value: obj
                });
                ops.push({
                  type: 'put',
                  key:
                    addr_by_time_prefix +
                    integer2LexString(address.time.getTime()) +
                    '/' +
                    addressId,
                  value: addressId
                });
    ...
    if (queue.length < 250 && currentTime - lastRefreshTime > 1000 * 15) {
      ...
      queue.length < 500 &&
      currentTime - lastRefreshTime > 1000 * 30
    ) {
      //every 30 seconds
      refreshQueue();
    } else if (
      queue.length < 1000 &&
      currentTime - lastRefreshTime > 1000 * 60
    ) {
      //every minute
      refreshQueue();
    } else if (
      queue.length < 2000 &&
      currentTime - lastRefreshTime > 1000 * 60 * 2
    ) {
      //every 2 minutes
      refreshQueue();
    } else if (
      queue.length < 4000 &&
      currentTime - lastRefreshTime > 1000 * 60 * 4
    ) {
      //every 4 minutes
      refreshQueue();
    } else if (currentTime - lastRefreshTime > 1000 * 60 * 8) {
      //every 8 minutes
      refreshQueue();
    }
  }
    ...
    createQueue(function(data) {
      if (data.length === 0) {
        ...
        seedNodes.forEach(host => {
          queue.push({
            host: host,
            port: ravencore_lib.Networks.get(network_name).port,
            nextConnection: 0
          });
        });
      } else {
